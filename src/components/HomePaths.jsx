...
export default function HomePaths() {
  ...
  return <>
    <section className="home-paths" aria-labelledby="home-paths-heading">
      ...
      <div className="home-path-list">{paths.map(({ number, icon: Icon, title, copy, link, label }, index) => <motion.article className="home-path" key={number} initial={reducedMotion ? false : { opacity: 0, y: 18 }} whileInView={reducedMotion ? {} : { opacity: 1, y: 0 }} viewport={{ once: true, amount: .25 }} transition={{ duration: .55, delay: index * .08, ease: [0.16, 1, .3, 1] }}><div className="home-path-number">{number}</div><div className="home-path-icon"><Icon size={22} aria-hidden="true" /></div><div><h3>{title}</h3><p>{copy}</p><Link to={link}>{label} <ArrowRight size={16} aria-hidden="true" /></Link></div></motion.article>)}</div>
    </section>
