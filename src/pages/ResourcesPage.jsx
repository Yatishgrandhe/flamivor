  ...
  return (
    <main className="resources-page">
      ...
      <section className="resources-index" aria-labelledby="resource-index-heading">
        ...
        <div className="resource-records">{resources.slice(1).map((resource, index) => { const Icon = resource.icon; return <motion.article className={`resource-record resource-record-${resource.accent}`} key={resource.id} {...enter(index * .08)}><div className="resource-record-number">{resource.id}</div><div className="resource-record-image"><img src={resource.image} alt={`Preview of Flamivor ${resource.title}`} loading="lazy" /></div><div className="resource-record-copy"><p><Icon size={15} aria-hidden="true" /> {resource.type}</p><h3>{resource.title}</h3><span>{resource.desc}</span></div><div className="resource-record-status">Links coming soon <ArrowRight size={16} aria-hidden="true" /></div></motion.article> })}</div>
      </section>
